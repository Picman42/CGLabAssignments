...
Raytracer::Raytracer(SceneParser *s, 
    ...
    bool use_grid, int grid_nx, int grid_ny, int grid_nz):
    ...
    UseGrid(use_grid)
{
    ...
    if (use_grid)
    {
        ...
        SceneGrid = new Grid(bbox, grid_nx, grid_ny, grid_nz);
...
{   
    ...
    if (is_hit)
    {
        ...
        if (can_shade)
        {
            ...
            // Reflection
            Vec3f col_refl = mat->getReflectionColor();
            ...
            // Refraction
            Vec3f col_refrac = mat->getTransparentColor();
            float refrac = mat->getRefractionIndex();
